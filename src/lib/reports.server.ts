...
export type CreateReportCoreOptions = {
	input: CreateReportInput;
	actorUserId: string;
	source?: "HUMAN" | "ENVIRONMENT_MONITOR";
...
export async function createReportCore({
...
}: CreateReportCoreOptions): Promise<CreateReportResult> {
	const data = validateCreateReportInput(uncheckedInput);
	const title = data.title.trim();
	const description = data.description.trim();
	const category = data.category.trim();
	const urgency = data.urgency?.trim() || "Sedang";
	const locationName = data.locationName.trim();
	const resolvedDeduplicationKey =
		deduplicationKey ?? `${source.toLowerCase()}:${crypto.randomUUID()}`;
	...
	const report = await prisma.report.create({
		data: {
			...
			sourceConfidence,
			sourceMetadata,
			deduplicationKey: resolvedDeduplicationKey,
			observedAt,
			accuracyRadiusMeters,
				...
				? {
						ecolensAnalysis: {
							create: {
								...
								suggestedDescription: data.ecolensAnalysis.suggestedDescription,
...
export async function createHumanReport(data: CreateReportInput) {
	const session = await ensureSession();
	return createReportCore({
		input: data,
...
async function refreshOwnedReportAssessment(
	...
	if (
		(!force && currentAssessment?.status === "COMPLETE") ||
		(!force && currentAssessment?.status === "PARTIAL") ||
		(!force && currentAssessment?.status === "FAILED" && !allowFailed) ||
		(!force && retryCooldownActive(report))
	) {
		return resultToPublicView(report);
	}
...
export async function refreshAutomaticReportAssessment(
	reportId: string,
	rateLimitKey: string,
): Promise<CreateReportResult | null> {
	const report = await prisma.report.findFirst({
		where: { id: reportId, source: "ENVIRONMENT_MONITOR" },
		include: reportResultInclude,
	});
	if (!report) return null;

	return refreshOwnedReportAssessment(report, rateLimitKey, true, true);
}

export async function refreshHumanReportAssessment(reportId: string) {
	const session = await ensureSession();
	const report = await loadOwnedReport(reportId, session.user.id);
